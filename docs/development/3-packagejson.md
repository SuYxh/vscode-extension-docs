...
在详细介绍 vscode 插件开发细节之前，这里我们先详细介绍一下 vscode 插件的`package.json`写法，但是建议先只需要随便看一下，了解个大概，等后面讲到具体细节的时候再回过头来看。
...
插件在`VS Code`中默认是没有被激活的，哪什么时候才被激活呢？就是通过`activationEvents`来配置，目前支持一下 8 种配置：
...
都比较好懂，我就不做一一介绍了，举个例子，如果我配置了`onLanguage:javascript`，那么只要我打开了 JS 类型的文件，插件就会被激活。

重点说一下`*`，如果配置了`*`，只要一启动 vscode，插件就会被激活，为了出色的用户体验，官方不推荐这么做。看到这里相信大家知道了我们前面 HelloWord 里面为啥要配置`onCommand`了吧。
...
- [`jsonValidation`](https://code.visualstudio.com/docs/extensionAPI/extension-points#_contributesjsonvalidation)：自定义 JSON 校验
- [`views`](https://code.visualstudio.com/docs/extensionAPI/extension-points#_contributesviews)：左侧侧边栏视图
- [`viewsContainers`](https://code.visualstudio.com/docs/extensionAPI/extension-points#_contributesviewscontainers)：自定义 activitybar
